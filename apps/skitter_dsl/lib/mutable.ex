...
defmodule Skitter.DSL.Mutable do
  @moduledoc false
  # Ugly yet genius code which makes a variable "mutable". Updates to this variable will be
  # visible inside the entire block, even in the presence of control flow constructs.
  ...
  defp add_default_branches(node = {op, env, args}) when op in [:if, :unless] do
    ...
    if :else in Keyword.keys(branches) do
      node
    else
